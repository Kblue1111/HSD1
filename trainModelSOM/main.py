...
def loadFile(project, version):
    try:
        ...
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "newsomResult.json"), 'r') as f:
    ...
    try:
        ...
        with multiprocessing.Manager() as manager:
            ...
            with multiprocessing.Pool(num_workers) as pools:
                ...
                # # 将数据存储在内存中
                # compressed_data1 = pickle.dumps(data)
                
                # # 将所有数据一次性写入到文件中
                # with gzip.open(f"../kxl/{project}_{version}_data.pkl.gz", "wb") as f:
                #     f.write(compressed_data1)
...
def randomForestModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        ...
        if os.path.isfile(os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_rf_{i}_2_newModel")):
            rf_clf = loadModel(os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_rf_{i}_2_newModel"))
        else:
            # 定义随机森林分类器
            ...
            if saveFile:
                saveModel(rf_clf, os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_rf_{i}_2_newModel"))
        end_train = datetime.now()
        train_duration = round((end_train - start_train).total_seconds(), 4)  # 计算训练时间
        ...
        predict_duration = round((end_predict - start_predict).total_seconds(), 4)  # 计算预测时间
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_rf_{i}_2_newModel.json"),"w") as f:
        ...
        print({"train_time": train_duration, "predict_time": predict_duration})
        return result
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        line_number = exc_tb.tb_lineno
...
def svmModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        X_train = kill_data['X_train']
        X_test = kill_data['X_test']
        y_train = kill_data['y_train']
        ...
        if os.path.isfile(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_svm_{i}_Model")):
            svm_clf = loadModel(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_svm_{i}Model"))
        else:
            n_estimators = 10
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_svm_{i}Model.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def logisticRegressionModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        X_train = kill_data['X_train']
        X_test = kill_data['X_test']
        y_train = kill_data['y_train']
        ...
        checkAndCreateDir(os.path.join(
            SOMfaultlocalizationResultPath, project, version, "Model"))
        if os.path.isfile(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_lr_{i}_Model")):
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_lr_{i}_Model.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def knnModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        X_train = kill_data['X_train']
        X_test = kill_data['X_test']
        y_train = kill_data['y_train']
        ...
        checkAndCreateDir(os.path.join(
            SOMfaultlocalizationResultPath, project, version, "Model"))
        if os.path.isfile(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_knn_{i}_Model")):
            knn_clf = loadModel(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_knn_{i}Model"))
        else:
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_knn_{i}Model.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def naiveBayesModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        X_train = kill_data['X_train']
        X_test = kill_data['X_test']
        y_train = kill_data['y_train']
        ...
        checkAndCreateDir(os.path.join(
            SOMfaultlocalizationResultPath, project, version, "Model"))
        if os.path.isfile(os.path.join(
                SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_nb_{i}_Model")):
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_nb_{i}_Model.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def executeModel(modelName, featureVectList, passLabelList, killLabelList, museKillLabelList, pkMap1, pkMap2, faultFileName1, faultFileName2,
    ...
    try:
        ...
        killResult = randomForestModel(featureVectList, killLabelList, saveFile, project, version, "kill", kill_data, i)
        ...
        susResult = calMbfl(project, version, passLabelList, killResult, pkMap1, pkMap2, faultFileName1, faultFileName2, testNum,
                            "pred_susFunc_" + str(modelName).split(" ")[1] + f"_{i}_3.json", i)
        topNResultBest,topNResultAverage,topNResultWorst = calTopNMbfl(project, version, susResult, "pred_topN_" + str(modelName).split(" ")[1] + f"_{i}_3.json")

        MFR,MAR = calMFRandMAR(project, version, topNResultWorst, "pred_TMP_" + str(modelName).split(" ")[1] + f"_{i}_3.json")

        print(project, version, str(modelName).split(" ")[1], i, "success")
...
if __name__ == "__main__":
    ...
    for i in range(3,4):
        ...
        kill_data = data_split(featureVectList,killLabelList,0.01 * i)
