...
def getTrainData(project, version):
    try:
        ...
        #region 存储杀死信息四元组和怀疑度分数
        muTuple = [] #以逗号隔开，依次是：变异体编号、Akf, Anf, Akp, Anp、各个公式的怀疑度
        muTuplePath = os.path.join(faultlocalizationResultPath, project, version, 'muTuple.txt')

        for muResult in muResults:
            if all(not muResult['passList']['type{}'.format(i + 1)] for i in range(4)) and all(not muResult['killList']['type{}'.format(i + 1)] for i in range(4)):
                muResult['status'] = 0
            if muResult['status'] != 0:
                tmp = str(muResult['index']) + ','
                passInfo = muResult['passList']['type4']
                killInfo = muResult['killList']['type4']
                Anp = 0
                Anf = 0
                Akp = 0
                Akf = 0
                for index in range(len(passInfo)):
                    if passInfo[index] == 1:
                        if killInfo[index] == 1:
                            Akf += 1
                        else:
                        ...
                        if killInfo[index] == 1:
                            ...
                            Anp += 1
                tmp = tmp + str(Akf) + ',' + str(Anf) + ',' + str(Akp) + ',' + str(Anp) + ','
                for method in mbflMethods:
                    v = method(Akf, Anf, Akp, Anp)
                    tmp = tmp + str(v) + ','
                tmp = tmp[:-1]
                muTuple.append(tmp)
            
        
        with open(muTuplePath, 'w') as file:
            for item in muTuple:
                file.write("%s\n" % item)
        # endregion
...
def calMbfl(project, version, passList, killList, pkMap, faultFileName, testNum, fileName):
    ...
    try:
        ...
        #region 存储杀死信息四元组和怀疑度分数
        muTuplePred = [] #以逗号隔开，依次是：变异体编号、Akf, Anf, Akp, Anp、各个公式的怀疑度
        muTuplePath = os.path.join(faultlocalizationResultPath, project, version, 'muTuple.txt')
        muTuplePredPath = os.path.join(faultlocalizationResultPath, project, version, 'muTuplePred.txt')
        with open(muTuplePath, 'r') as file:
            # 读取文件的每一行并存储在一个列表中
            muTuple = file.readlines()
            ...
            for index in range(0, testNum):
                if passList[index + i * testNum] == 1:
                    ...
                    if killList[index + i * testNum] == 1:
                        ...
                        Anp += 1
            tmp = muTuple[i].split(',')[0] + ',' + str(Akf) + ',' + str(Anf) + ',' + str(Akp) + ',' + str(Anp) + ','
            for method in mbflMethods:
                v = method(Akf, Anf, Akp, Anp)
                tmp = tmp + str(v) + ','
                ...
                if susResult[faultFileName[i]][str(method).split(" ")[1]].get(pkMap[i]) == None:
                    ...
                    susResult[faultFileName[i]][str(method).split(" ")[1]][pkMap[i]] = max(
                        susResult[faultFileName[i]][str(method).split(" ")[1]][pkMap[i]], method(Akf, Anf, Akp, Anp))
            tmp = tmp[:-1]
            muTuplePred.append(tmp)
        
        with open(muTuplePredPath, 'w') as file:
            for item in muTuplePred:
                file.write("%s\n" % item)
        
        #以逗号隔开，依次是：变异体编号、Akf, Anf, Akp, Anp、各个公式的怀疑度
        muTupleDiff = []
        muTuplePath = os.path.join(faultlocalizationResultPath, project, version, 'muTuple.txt')
        muTupleDiffPath = os.path.join(faultlocalizationResultPath, project, version, 'muTupleDiff.txt')
        with open(muTuplePath, 'r') as file:
            muTuple = file.readlines()

        for i in range(len(muTuple)):
            muTuple_array = [float(x) for x in muTuple[i].strip().split(',')]
            muTuplePred_array = [float(x) for x in muTuplePred[i].strip().split(',')]
            tmp = str(int(muTuple_array[0])) + ','
            for j in range(1,len(muTuple_array)):
                tmp = tmp + str(muTuple_array[j] - muTuplePred_array[j]) + ','
            tmp = tmp[:-1]
            muTupleDiff.append(tmp)


        with open(muTupleDiffPath, 'w') as file:
            for item in muTupleDiff:
                file.write("%s\n" % item)
...
if __name__ == "__main__":
    ...
    logger = logger_config(log_path='main_showdiff.log')
    ...
    for project in projectList.keys():
        for versionNum in range(1, projectList[project] + 1):
            ...
            kill_data = data_split(featureVectList,killLabelList,0.1)
