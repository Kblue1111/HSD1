...
def randomForestModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        if os.path.isfile(os.path.join(faultlocalizationResultPath, project, version, "Model", mode + "_rfModel_based_sbfl")):
            rf_clf = loadModel(os.path.join(faultlocalizationResultPath, project, version, "Model", mode + "_rfModel_based_sbfl"))
        else:
            # 定义随机森林分类器
            ...
            if saveFile:
                saveModel(rf_clf, os.path.join(faultlocalizationResultPath, project, version, "Model", mode + "_rfModel_based_sbfl"))
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_rfModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))

        result = np.empty(len(featureVectList), dtype=y_train.dtype)
        result[indx_train] = y_train
        result[indx_test] = y_pred
...
def svmModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_svmModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def logisticRegressionModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_lrModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def knnModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_knnModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def naiveBayesModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_nbModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def mlpModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        # 将字典写入json文件
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_mlpModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def cnnModel(featureVectList, labelList, saveFile, project, version, mode, kill_data):
    try:
        ...
        checkAndCreateDir(os.path.join(
            faultlocalizationResultPath, project, version, "Model"))
        if os.path.isfile(os.path.join(
                faultlocalizationResultPath, project, version, "Model", mode + "_cnnModel_based_sbfl")):
            mlp_clf = loadModel(os.path.join(
                faultlocalizationResultPath, project, version, "Model", mode + "_cnnModel_based_sbfl"))
        else:
            # # cnn
        ...
        with open(os.path.join(faultlocalizationResultPath, project, version, "ModelMetrics", mode + "_cnnModel_based_sbfl.json"),"w") as f:
            f.write(json.dumps(results, indent=2))
...
def calMbfl(project, version, passList, killList, pkMap, faultFileName, testNum, fileName):
    ...
    try:
        ...
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, 'predSuspicious_BasedSbfl'))

        suspiciousFirstOrderPath = os.path.join(faultlocalizationResultPath, project, version, 'predSuspicious_BasedSbfl', fileName)
        susResult = dict()
        susFunc = dict()
...
def calTopNMbfl(project, version, susResult, FileName):
    try:
        ...
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionBest_BasedSbfl"))
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionAverage_BasedSbfl"))
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionWorst_BasedSbfl"))
        with open(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionBest_BasedSbfl", FileName), 'w') as f:
            f.write(json.dumps(topNResultBest, indent=2))
            f.close()
        with open(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionAverage_BasedSbfl", FileName),
                  'w') as f:
            f.write(json.dumps(topNResultAverage, indent=2))
            f.close()
        with open(os.path.join(faultlocalizationResultPath, project, version, "predTopNFunctionWorst_BasedSbfl", FileName), 'w') as f:
...
def calMFRandMAR(project, version, topNResult, FileName):
    try:
        ...
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, "predMFR_BasedSbfl"))
        checkAndCreateDir(os.path.join(faultlocalizationResultPath, project, version, "predMAR_BasedSbfl"))
        with open(os.path.join(faultlocalizationResultPath, project, version, "predMFR_BasedSbfl", FileName.replace('TMP','MFR')), 'w') as f:
            f.write(json.dumps(MFR, indent=2))
            f.close()
        with open(os.path.join(faultlocalizationResultPath, project, version, "predMAR_BasedSbfl", FileName.replace('TMP','MAR')), 'w') as f:
...
def getMutantBasedSbfl(rate = 0.1):
    # 获取变异体个数
    with open(os.path.join(faultlocalizationResultPath, project, version, "muInfo.json"), 'r') as f:
        muInfo = json.load(f)
        
    with open(os.path.join(outputCleanPath, project, version, "HugeToFile.txt"), 'r') as f:
        HugeToFile = f.readlines()

    with open(os.path.join(outputCleanPath, project, version, "faultHuge_Function.txt"), 'r') as f:
        faultHuge_Function = json.load(f)

    with open(os.path.join('/home/fanluxi/d4j/hugeToFunction', project, version, "HugetoFunction.txt"), 'r') as f:
        HugetoFunction = f.readlines()
        
    muNum = int(len(muInfo)* rate)
    
    nowNum = 0
    muInde = list()
    for key in faultHuge_Function:
        arr = faultHuge_Function[key]
        hugeCodeLineInfo = key.split('java/')[1]
        # Time 1b 特殊处理
        if project == 'Time' and version == '1b':
            hugeCodeLineInfo = '/time' + hugeCodeLineInfo.split('time')[1]
        ...
        for funcNum in arr:
            # 拿.java文件的起始行
            startLine = 0
            for line in HugeToFile:
                startLine = startLine + 1
                if hugeCodeLineInfo in line:
                    break;
            # 拿错误函数的起始行
            begin = -1
            end = -1
            cnt = 0
            for line in HugetoFunction:
                cnt = cnt + 1
                if int(line) == funcNum and begin == -1:
                    begin = cnt - startLine + 1
                if begin != -1 and int(line) != funcNum:
                    end = cnt - startLine
                    break
            if end == -1:
                end = cnt - startLine + 1
            ...
            for item in muInfo:
                if item['relativePath'] != key[1:]:
                    continue
                if item['linenum'] >= begin and item['linenum'] <= end:
...
if __name__ == "__main__":
    ...
    logger = logger_config(log_path='trickPMBFL.log')
    ...
    for project in projectList.keys():
        for versionNum in range(1, projectList[project] + 1):
            ...
            muInde = getMutantBasedSbfl(0.2)
            print(muInde)

            kill_data = data_split(muInde, testNum, featureVectList, killLabelList, 0.2)
