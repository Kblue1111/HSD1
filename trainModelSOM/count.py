...
def countTonN(root_path, project, mode):
    ...
    with open('./predTopN/' + mode + '/' + project + ".json", 'w') as f:
...
def countMAR(root_path, project, mode):
    ...
    if 'BasedSbfl' in mode:
        project = project + '_basedSBFL'
    with open("./predMAR/" + project + ".json", 'w') as f:
        f.write(json.dumps(projectStatistics, indent=2))
...
def countMFR(root_path, project, mode):
    ...
    if 'BasedSbfl' in mode:
        project = project + '_basedSBFL'
    with open("./predMFR/" + project + ".json", 'w') as f:
        f.write(json.dumps(projectStatistics, indent=2))
...
def countModelMetrics(root_path, project):
    ...
    for filename in projectStatistics.keys():
        for key in projectStatistics[filename].keys():
            projectStatistics[filename][key] /= count
    # print(projectStatistics)
    with open("./ModelMetrics/" + project + ".json", 'w') as f:
        f.write(json.dumps(projectStatistics, indent=2))
...
if __name__ =="__main__":
    ...
    rootdir = "/home/fanluxi/pmbfl/SOMfaultlocalizationResult/"
    # project = "Time"
    # countTonN(rootdir, project, 'TopNFunctionWorst')
    # countTonN(rootdir, project, 'TopNFunctionAverage')
    # countTonN(rootdir, project, 'TopNFunctionBest')
    # countMFR(rootdir, project, 'TopNFunctionWorst')
    # countMAR(rootdir, project, 'TopNFunctionWorst')
    for project in projectList.keys():
        ...
        # 基于SBFL的PMBFL
        # countTonN(rootdir, project, 'predTopNFunctionBest_BasedSbfl')
        # countTonN(rootdir, project, 'predTopNFunctionAverage_BasedSbfl')
        # countTonN(rootdir, project, 'predTopNFunctionWorst_BasedSbfl')
        # print('TOP N finish')
        # countMAR(rootdir, project,'predMAR_BasedSbfl')
        # print('MAR finish')
        # countMFR(rootdir, project,'predMFR_BasedSbfl')
        # print('MFR finish')
