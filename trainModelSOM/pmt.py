...
def getFeatureVect(project, version, info, result, staticFea, dynamicFea, crFea, sfFea, ssFea):
    try:
        ...
        dynamicFea = [[0 for _ in range(len(row))] for row in dynamicFea]
...
def load_compressed_data(project, version):
    if os.path.exists(f"../pmt/{project}_{version}_data.pkl.gz"):
        with gzip.open(f"../pmt/{project}_{version}_data.pkl.gz", "rb") as f:
    ...
    try:
        ...
        with multiprocessing.Manager() as manager:
            ...
            with multiprocessing.Pool(num_workers) as pools:
                ...
                # 将所有数据一次性写入到文件中
                with gzip.open(f"../pmt/{project}_{version}_data.pkl.gz", "wb") as f:
...
def randomForestModel(featureVectList, labelList, saveFile, project, version, mode, kill_data, i):
    try:
        ...
        if os.path.isfile(os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_pmt_{i}Model")):
            rf_clf = loadModel(os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_pmt_{i}Model"))
        else:
            # 定义随机森林分类器
            ...
            if saveFile:
                saveModel(rf_clf, os.path.join(SOMfaultlocalizationResultPath, project, version, "Model", mode + f"_pmt_{i}Model"))
        ...
        # 将字典写入json文件
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, "ModelMetrics", mode + f"_pmt_{i}Model.json"),"w") as f:
...
def executeModel(modelName, featureVectList, passLabelList, killLabelList, museKillLabelList, pkMap1, pkMap2, faultFileName1, faultFileName2,
    ...
    try:
        ...
        susResult = calMbfl(project, version, passLabelList, killResult, pkMap1, pkMap2, faultFileName1, faultFileName2, testNum,
                            f"pred_susFunc_pmt_{i}.json", i)
        topNResultBest,topNResultAverage,topNResultWorst = calTopNMbfl(project, version, susResult, f"pred_topN_pmt_{i}.json")

        MFR,MAR = calMFRandMAR(project, version, topNResultWorst, f"pred_TMP_pmt_{i}.json")
...
def calMbfl(project, version, passList, killList, pkMap1, pkMap2, faultFileName1, faultFileName2, testNum, fileName, i):
    ...
    try:
        ...
        muInfo = random.sample(muInfo, round(len(killList) / testNum  * 0.001 * i))
        new_muResult = []
        for item in muInfo:
