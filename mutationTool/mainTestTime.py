...
def FOM(project, version, configData):
    ...
    try:
        ...
        logging.info(f"{project} Time Cost: {current_time2 - current_time1}  " )
...
if __name__ == "__main__":
    ...
    logger = logger_config(log_path="mainTestTime.log")
    ...
    try:
        with multiprocessing.Pool(5) as executor:
            for projectDir in projectList.keys():
                # projectDir = "Chart"
                projectDir = "Closure"
                # projectDir = "Lang"
                # projectDir = "Math"
                # projectDir = "Time"
                versionDir =  "1b"
                if projectDir == "Math" or projectDir == "Closure":
                    versionDir =  "3b"
                current_time = datetime.now()
                formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S")
                print(formatted_time)
                logging.info(f"{projectDir} {versionDir} start")
