...
def calEXAM(project, version, dire, FileName):
    try:
        examResults = dict()  # 用于存储每个方法的EXAM值
        with open(
            "../../d4j/outputClean/" + project + "/" + version + "/FunctionList.txt",
            "r",
        ) as f:
            FunctionList = f.readlines()

        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, dire, FileName), "r") as f:
        ...
        for method in mbflMethods:
            ...
            for faultKey in faultFunc:
                ...
                # region 创建字典
                if examResults.get(key) is None:
                    examResults[key] = dict()
                if examResults[key].get(line) is None:
                    examResults[key][line] = dict()
                # endregion

                if newSusResult.get(faultKey) is None:
                    examResults[key][line][method_name] = 1
                    continue
                ...
                if startFlagIndex == -1 or endFlagIndex == -1:
                    ...
                    examResults[key][line][method_name] = (
                        startFlagIndex + endFlagIndex
                    ) / 2/len(FunctionList) *100
                # examResults[key][line][method_name] = endFlagIndex / len(FunctionList) * 10
                examResults[key][line][method_name] = endFlagIndex / 30
                if examResults[key][line][method_name] == 1:
                    examResults[key][line][method_name] = 0
        result = dict()
        for item, value in examResults.items():
            for line, methods in value.items():
                for method, v in methods.items():
                    if result.get(method) is None:
                        result[method] = list()
                    result[method].append(v)
        for method, item in result.items():
            a = list()
            a.append(min(result[method]))
            result[method] = a
        checkAndCreateDir(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, f"EXAM{dire}"
            )
        )
        print(
            os.path.join(
...
def countTotalEXAM(directory):
    output_file = os.path.join(directory, 'total.json')
    output_avg_file = os.path.join(directory, 'average.json')
    if os.path.exists(output_file):
        # 删除文件
        os.remove(output_file)
    # 初始化一个空字典来存储所有数据
    ...
    for filename in os.listdir(directory):
        ...
        with open(file_path, "r") as f:
            ...
            for indicator, values in file_data.items():
                ...
                for key, value in values.items():
                    if key not in sum_data[indicator]:
                        sum_data[indicator][key] = list()
                    sum_data[indicator][key].extend(value)
                    count_data[indicator][key].extend(len(value))

    avg_data = {}
    for indicator, keys in sum_data.items():
        avg_data[indicator] = {}
        for key, sum_value in keys.items():
            avg_data[indicator][key] = sum_value / count_data[indicator][key] if count_data[indicator][key] > 0 else 0

    with open(output_file, "w") as f:
        json.dump(sum_data, f, indent=4)  # 缩进为4个空格，可选
        
    with open(output_avg_file, "w") as f:
        json.dump(avg_data, f, indent=4)
        
    

def calSOM(project, version):
    """高阶所有变异体的平均"""
    try:
        subSOMPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/subSOMInfo.json"
        somResultPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/somResult.json"
        if not os.path.exists(somResultPath):
            return
        with open(subSOMPath, "r") as f:
            subSOM = json.load(f)
        with open(somResultPath, "r") as f:
            somResult = json.load(f)
        susResult = {}
        num = dict()
        for j in range(1, 5):
            ...
            if somResult == None:
                somResult = []
            for i in range(0, len(somResult)):
                if somResult[i]["status"] == 0:
                    continue
                Anp = 0
                Anf = 0
                Akp = 0
                Akf = 0
                if susResult[f"type{j}"].get(subSOM[i]["relativePath1"]) == None:
                    susResult[f"type{j}"][subSOM[i]["relativePath1"]] = dict()
                for index in range(0, len(somResult[i]["passList"][f"type{j}"])):
                    if somResult[i]["passList"][f"type{j}"][index] == 1:
                        if somResult[i]["killList"][f"type{j}"][index] == 1:
                            Akf += 1
                        else:
                            Anf += 1
                    else:
                ...
                for method in mbflMethods:
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]].get(
                            str(method).split(" ")[1]
                        )
                        == None
                    ):
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ] = dict()
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ].get(somResult[i]["linenum1"])
                        == None
                    ):
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum1"]] = method(Akf, Anf, Akp, Anp)
                        num[somResult[i]["linenum1"]] = 1
                    # elif susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                    #     str(method).split(" ")[1]
                    # ][somResult[i]["linenum1"]] > method(Akf, Anf, Akp, Anp):
                    else:
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                    ...
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ].get(somResult[i]["linenum2"])
                        == None
                    ):
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum2"]] = method(Akf, Anf, Akp, Anp)
                        num[somResult[i]["linenum2"]] = 1
                    # elif susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                    #     str(method).split(" ")[1]
                    # ][somResult[i]["linenum2"]] > method(Akf, Anf, Akp, Anp):
                    else:
            ...
            for item in susResult[f"type{j}"].keys():
                for method in mbflMethods:
                    for line_num in susResult[f"type{j}"][item][str(method).split(" ")[1]]:
                        susResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] /= num[line_num]
            for item in susResult[f"type{j}"].keys():
                for method in mbflMethods:
        ...
        checkAndCreateDir(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, "susStatement"
            )
        )
        with open(f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM.json", "w") as f:
            f.write(json.dumps(susResult, indent=2))
        # calTopNMbfl(project, version, susResult, file, "Statement")
        if ip != "202.4.130.30":
            sftp_upload(
                "202.4.130.30",
                "fanluxi",
                password,
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx",
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx",
            )
        return susResult
    except Exception as e:
...
def calSOM_czx(project, version):
    """一阶和高阶所有所有变异体的平均"""
    try:
        ...
        for j in range(1, 5):
            ...
            for i in range(0, len(muResult)):
                ...
                Akp = 0
                Akf = 0
                if susResult[f"type{j}"].get(muInfo[i]["relativePath"]) == None:
                    susResult[f"type{j}"][muInfo[i]["relativePath"]] = dict()
                for index in range(0, len(muResult[i]["passList"][f"type{j}"])):
                    if muResult[i]["passList"][f"type{j}"][index] == 1:
                        if muResult[i]["killList"][f"type{j}"][index] == 1:
                            Akf += 1
                        else:
                            Anf += 1
                    else:
                        if muResult[i]["killList"][f"type{j}"][index] == 1:
                            ...
                            Anp += 1
                for method in mbflMethods:
                    if (susResult[f"type{j}"][muInfo[i]["relativePath"]].get(str(method).split(" ")[1])== None):
                        susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]] = dict()
                    if (susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]].get(muResult[i]["linenum"])== None):
                        susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] = method(Akf, Anf, Akp, Anp)
                        num[muResult[i]["linenum"]] = 1
                    else:
                        susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] += method(Akf, Anf, Akp, Anp)
                        num[muResult[i]["linenum"]] += 1
            for item in susResult[f"type{j}"].keys():
        ...
        with open(f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx.json", "w") as f:
            f.write(json.dumps(susResult, indent=2))
        # calTopNMbfl(project, version, susResult, file, "Statement")
        if ip != "202.4.130.30":
            sftp_upload(
...
def calSOM_czx2(project, version):
    "一阶和高阶所有变异体的怀疑度最大值减去平均值"
    try:
        muInfoPath = f"{faultlocalizationResultPath}/{project}/{version}/muInfo.json"
        muResultPath = f"{faultlocalizationResultPath}/{project}/{version}/muResult.json"
        subSOMPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/subSOMInfo.json"
        ...
        for j in range(1, 5):
            ...
            for i in range(0, len(somResult)):
                ...
                if susResult[f"type{j}"].get(subSOM[i]["relativePath1"]) == None:
                    susResult[f"type{j}"][subSOM[i]["relativePath1"]] = dict()
                    maxSus[f"type{j}"][subSOM[i]["relativePath1"]] = dict()
                for index in range(0, len(somResult[i]["passList"][f"type{j}"])):
                    if somResult[i]["passList"][f"type{j}"][index] == 1:
                        if somResult[i]["killList"][f"type{j}"][index] == 1:
                            Akf += 1
                ...
                for method in mbflMethods:
                    if (
                        ...
                        maxSus[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ] = dict()
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                    ...
                    # ][somResult[i]["linenum1"]] > method(Akf, Anf, Akp, Anp):
                        ...
                        maxSus[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum1"]] = max(method(Akf, Anf, Akp, Anp), maxSus[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum1"]]) 
                        num[somResult[i]["linenum1"]] += 1
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                    ...
                    # ][somResult[i]["linenum2"]] > method(Akf, Anf, Akp, Anp):
                        ...
                        maxSus[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum2"]] = max(method(Akf, Anf, Akp, Anp), maxSus[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum2"]])
                        num[somResult[i]["linenum2"]] += 1
            for i in range(0, len(muResult)):
                if muResult[i]["status"] == 0:
                    continue
                ...
                for method in mbflMethods:
                    ...
                    if (susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]].get(muResult[i]["linenum"])== None):
                        ...
                        maxSus[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] = method(Akf, Anf, Akp, Anp)
                        
                        num[muResult[i]["linenum"]] = 1
                    else:
                        susResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] += method(Akf, Anf, Akp, Anp)
                        maxSus[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] = max(method(Akf, Anf, Akp, Anp), maxSus[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]])
                        num[muResult[i]["linenum"]] += 1
            for item in susResult[f"type{j}"].keys():
                for method in mbflMethods:
                    for line_num in susResult[f"type{j}"][item][str(method).split(" ")[1]]:
                        susResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] /= num[line_num]
                        susResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] = maxSus[f"type{j}"][item][str(method).split(" ")[1]][line_num] - susResult[f"type{j}"][item][str(method).split(" ")[1]][line_num]
            for item in susResult[f"type{j}"].keys():
                for method in mbflMethods:
                    susResult[f"type{j}"][item][str(method).split(" ")[1]] = dict(
                        sorted(
        ...
        with open(f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx2.json", "w") as f:
            f.write(json.dumps(susResult, indent=2))
        # calTopNMbfl(project, version, susResult, file, "Statement")
        if ip != "202.4.130.30":
            sftp_upload(
                ...
                "fanluxi",
                password,
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx2",
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx2",
            )
        return susResult
    except Exception as e:
...
def calSOM_czx3(project, version):
    "一阶变异体的怀疑度最大值和高阶变异体的怀疑度最大值之和"
    try:
        muInfoPath = f"{faultlocalizationResultPath}/{project}/{version}/muInfo.json"
        muResultPath = f"{faultlocalizationResultPath}/{project}/{version}/muResult.json"
        subSOMPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/subSOMInfo.json"
            ...
            for i in range(0, len(somResult)):
                ...
                for method in mbflMethods:
                    ...
                    # ][somResult[i]["linenum1"]] > method(Akf, Anf, Akp, Anp):
                        ...
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum1"]] = max(method(Akf, Anf, Akp, Anp), susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum1"]]) 
                    if (
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                    ...
                    # ][somResult[i]["linenum2"]] > method(Akf, Anf, Akp, Anp):
                        ...
                        susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum2"]] = max(method(Akf, Anf, Akp, Anp), susResult[f"type{j}"][subSOM[i]["relativePath1"]][
                            str(method).split(" ")[1]
                        ][somResult[i]["linenum2"]])
            for i in range(0, len(muResult)):
                if muResult[i]["status"] == 0:
                ...
                Akp = 0
                Akf = 0
                if fomResult[f"type{j}"].get(muInfo[i]["relativePath"]) == None:
                    fomResult[f"type{j}"][muInfo[i]["relativePath"]] = dict()
                for index in range(0, len(muResult[i]["passList"][f"type{j}"])):
                    if muResult[i]["passList"][f"type{j}"][index] == 1:
                        if muResult[i]["killList"][f"type{j}"][index] == 1:
                            Akf += 1
                        ...
                        if muResult[i]["killList"][f"type{j}"][index] == 1:
                            ...
                            Anp += 1
                for method in mbflMethods:
                    if (fomResult[f"type{j}"][muInfo[i]["relativePath"]].get(str(method).split(" ")[1])== None):
                        fomResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]] = dict()
                    if (fomResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]].get(muResult[i]["linenum"])== None):
                        
                        fomResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] = method(Akf, Anf, Akp, Anp)
                    else:
                        fomResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]] = max(method(Akf, Anf, Akp, Anp), fomResult[f"type{j}"][muInfo[i]["relativePath"]][str(method).split(" ")[1]][muResult[i]["linenum"]])
            for item in susResult[f"type{j}"].keys():
                if sumResult.get(f"type{j}") is None:
                    sumResult[f"type{j}"] = dict()
                for method in mbflMethods:
                    if sumResult[f"type{j}"].get(item) is None:
                        sumResult[f"type{j}"][item] = dict()
                    if sumResult[f"type{j}"][item].get(str(method).split(" ")[1]) is None:
                        sumResult[f"type{j}"][item][str(method).split(" ")[1]] = dict()
                    for line_num in susResult[f"type{j}"][item][str(method).split(" ")[1]]:
                        if sumResult[f"type{j}"][item][str(method).split(" ")[1]].get(line_num) is None:
                            sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] = 0
                        sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] += susResult[f"type{j}"][item][str(method).split(" ")[1]][line_num]
            for item in fomResult[f"type{j}"].keys():
                if sumResult.get(f"type{j}") is None:
                    sumResult[f"type{j}"] = dict()
                for method in mbflMethods:
                    if sumResult[f"type{j}"].get(item) is None:
                    ...
                    if sumResult[f"type{j}"][item].get(str(method).split(" ")[1]) is None:
                        sumResult[f"type{j}"][item][str(method).split(" ")[1]] = dict()
                    for line_num in fomResult[f"type{j}"][item][str(method).split(" ")[1]]:
                        if sumResult[f"type{j}"][item][str(method).split(" ")[1]].get(line_num) is None:
                            sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] = 0
                        sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] += fomResult[f"type{j}"][item][str(method).split(" ")[1]][line_num]
            for item in sumResult[f"type{j}"].keys():
                for method in mbflMethods:
                    sumResult[f"type{j}"][item][str(method).split(" ")[1]] = dict(
                        sorted(
                            sumResult[f"type{j}"][item][
                                str(method).split(" ")[1]
                            ].items(),
                            key=operator.itemgetter(1),
                            reverse=True,
        ...
        with open(f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx3.json", "w") as f:
            f.write(json.dumps(sumResult, indent=2))
        # calTopNMbfl(project, version, sumResult, file, "Statement")
        if ip != "202.4.130.30":
            sftp_upload(
                "202.4.130.30",
                "fanluxi",
                password,
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx3",
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx3",
            )
        return susResult
    except Exception as e:
...
def calSOM_czx4(project, version):
    "SBFL怀疑度+一阶变异体的怀疑度最大值和高阶变异体的怀疑度最大值之和"
    try:
        muInfoPath = f"{faultlocalizationResultPath}/{project}/{version}/muInfo.json"
        muResultPath = f"{faultlocalizationResultPath}/{project}/{version}/muResult.json"
        subSOMPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/subSOMInfo.json"
        somResultPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/somResult.json"
        sbflResultPath = f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/sbfl.json"
        if not os.path.exists(muResultPath):
            return
        if not os.path.exists(somResultPath):
            return
        ...
        with open(somResultPath, "r") as f:
            somResult = json.load(f)
        with open(sbflResultPath, "r") as f:
            sbflResult = json.load(f)
        susResult = {}
        fomResult = {}
        sumResult = {}
        maxSus = {}
                ...
                for method in mbflMethods:
                    ...
                    for line_num in fomResult[f"type{j}"][item][str(method).split(" ")[1]]:
                        if sumResult[f"type{j}"][item][str(method).split(" ")[1]].get(line_num) is None:
                            sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] = 0
                        sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] += fomResult[f"type{j}"][item][str(method).split(" ")[1]][line_num]
            for item in sbflResult.keys():
                if sumResult[f"type{j}"].get(item) is None:
                    susResult[f"type{j}"][item] = dict()
                    for method in mbflMethods:
                        if susResult[f"type{j}"][item][str(method).split(" ")[1]] is None:
                            sumResult[f"type{j}"][item][str(method).split(" ")[1]] = dict()
                        for line_num in sbflResult[item][str(method).split(" ")[1]]:
                            if sumResult[f"type{j}"][item][str(method).split(" ")[1]].get(line_num) is None:
                                sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] = 0
                            sumResult[f"type{j}"][item][str(method).split(" ")[1]][line_num] += sbflResult[item][str(method).split(" ")[1]][line_num]
        ...
        if ip != "202.4.130.30":
            sftp_upload(
                ...
                "fanluxi",
                password,
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx4",
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx4",
            )
        return susResult
    except Exception as e:
...
def calSOM_czx5(project, version):
    "一阶变异体的怀疑度最大值和高阶变异体的怀疑度最大值之和"
    try:
        muInfoPath = f"{faultlocalizationResultPath}/{project}/{version}/muInfo.json"
        muResultPath = f"{faultlocalizationResultPath}/{project}/{version}/muResult.json"
        ...
        with open(f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx5.json", "w") as f:
            f.write(json.dumps(sumResult, indent=2))
        # calTopNMbfl(project, version, sumResult, file, "Statement")
        if ip != "202.4.130.30":
            sftp_upload(
                ...
                "fanluxi",
                password,
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx5",
                f"{SOMfaultlocalizationResultPath}/{project}/{version}/susStatement/SOM_czx5",
            )
        return susResult
    except Exception as e:
...
def calSomMbfl(project, version, subSOM, somResult, file, muInfo, muResult):
    """
    通过变异体的执行矩阵和杀死矩阵计算语句怀疑度
    :param Fom: 变异体的信息,主要用到行号
    :param FomResult: 变异体的执行结果和杀死信息,数组形式存储,第一个是执行结果,第二个是杀死信息.
    ...
    try:
        suspiciousFirstOrderPath = os.path.join(
            SOMfaultlocalizationResultPath, project, version, "susStatement", file
        )
        # muInfoPath = f"{faultlocalizationResultPath}/{project}/{version}/muInfo.json"
        # with open(muInfoPath, "r") as f:
        #     muInfo = json.load(f)
...
def countSOMFunctionSus(project, version):
    try:
        with open(
            "../../d4j/hugeToFunction/"
            + project
        ...
        with open(
            ...
            FunctionList = f.readlines()
        with open(
            "../../d4j/outputClean/" + project + "/" + version + "/HugeToFile.txt", "r"
        ) as f:
            hugeToFile = f.readlines()
        ...
        for key in faultHuge_Function.keys():
            ...
            for line in faultHuge_Function[key]:
                ...
                count = (
                    sum(
                        1
                        for element in FunctionList[0:line]
                        if FunctionList[line].split(":")[0] in element
                    )
                    + 1
                )
                # fanluxi
                                ...
                                for line in sus[f"type{j}"][key][method].keys():
                                    for k in hugeToFiledict.keys():
                                        if k in key:
                                            break
                                    if (
                                        hugeToFiledict[k].get(str(int(line) - 1))
                                        == None
                                    ):
                                        continue
                                    count = hugeToFiledict[k][str(int(line) - 1)]
                                    if (
                                        functionSus[f"type{j}"][key][method].get(count)
                                        == None
                                    ):
                                        functionSus[f"type{j}"][key][method][
                                            count
                                        ] = sus[f"type{j}"][key][method][line]
                with open(
                    SOMfaultlocalizationResultPath
                    + "/"
                    + project
                    ...
                    f.write(json.dumps(functionSus, indent=2))
                logging.info("开始计算TopN")
                topNResultBest,topNResultAverage,topNResultWorst = calTopNMbfl(project, version, functionSus, file, "Function")
                logging.info("开始计算MFR和MAR")
                MFR,MAR = calMFRandMAR(project, version, topNResultWorst, file, "Function")
...
def calMFRandMAR(project, version, topNResult, FileName, mode):
    try:
        ...
        checkAndCreateDir(os.path.join(SOMfaultlocalizationResultPath, project, version, f"{mode}_MFR"))
        checkAndCreateDir(os.path.join(SOMfaultlocalizationResultPath, project, version, f"{mode}_MAR"))
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, f"{mode}_MFR", FileName), 'w') as f:
            f.write(json.dumps(MFR, indent=2))
            f.close()
        with open(os.path.join(SOMfaultlocalizationResultPath, project, version, f"{mode}_MAR", FileName), 'w') as f:
...
def calTopNMbfl(project, version, susResult, FileName, mode):
    try:
        ...
        with open(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, "falutFunction.json"
            ),
            "r",
        ) as f:
            faultLocalization = json.load(f)
        ...
        checkAndCreateDir(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, f"TopN{mode}Best"
            )
        )
        checkAndCreateDir(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, f"TopN{mode}Average"
            )
        )
        checkAndCreateDir(
            os.path.join(
                SOMfaultlocalizationResultPath, project, version, f"TopN{mode}Worst"
            )
        )
        with open(
            os.path.join(
